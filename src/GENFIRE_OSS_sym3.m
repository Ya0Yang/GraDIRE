...
function [rec, errK, Rfree_magnitude,bestErrors] = GENFIRE_OSS_sym3(numIterations,initialObject,support,measuredK,constraintInd_complex,constraintInd_magnitude,R_freeInd_mag)
% showFigure = 0;
...
%             iterationNum
        ...
        %if mod(totalIterationNum,10) == 0 && totalIterationNum > floor(numIterations/10)
            %centralRegion = sample(99:123,99:123,99:123); %this is the nonzero region of the support
            %centralRegionSymmetrized = symmEnforceCube(centralRegion); 
            %sample(99:123,99:123,99:123) = centralRegionSymmetrized; %replacing the interior region of the cube with symmetrized values
      sample = applySymmetry(sample,cubicSymmetryOperations);
