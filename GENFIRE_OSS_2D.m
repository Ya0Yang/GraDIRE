...
function [rec, errK, Rfree_magnitude,bestErrors] = GENFIRE_OSS_2D(numIterations,initialObject,support,measuredK,constraintInd_complex,constraintInd_magnitude,R_freeInd_mag)
% showFigure = 0;
showFigure = 1;
beta = 0.9;
numFilters = 10;
...
for filterNum = 1:numFilters
        ...
        kfilter=exp( -( ( ((sqrt((aa-Rcenter).^2+(bb-Ccenter).^2)).^2 ) ./ (2* sigmas((filterNum-1)*(numIterations/numFilters)+1).^2) )));
