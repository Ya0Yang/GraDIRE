...
function [rec, errK, Rfree_magnitude,bestErrors,symArray,recArray] = GENFIRE_OSS_sym9(numIterations,initialObject,support,measuredK,constraintInd_complex,constraintInd_magnitude,R_freeInd_mag)
% showFigure = 0;
load('cubicSymmetryOperations.mat');
symArray = {};
recArray = {};
...
%             iterationNum
     ...
     if mod(totalIterationNum,30) == 0 && totalIterationNum > floor(numIterations/10) && totalIterationNum < (numIterations - floor(numIterations/10))
      symmetrizedRegion = sample(99:123,99:123,99:123);
      symmetrizedRegion = applySymmetry(symmetrizedRegion,cubicSymmetryOperations);
      sample(99:123,99:123,99:123) = symmetrizedRegion;
      %sample(ind) = applySymmetry(sample(ind),cubicSymmetryOperations);
      %symArray = [symArray;sample];
     end
...
%         sample = alignByCOMy(sample);
        ...
        if errK(totalIterationNum)<bestErr&&totalIterationNum >store+2 %if current reconstruction has better error, update best error and best reconstruction
            ...
            %rec = applySymmetry(sample,cubicSymmetryOperations);
            rec = sample;
