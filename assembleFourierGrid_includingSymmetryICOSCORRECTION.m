function measuredK = assembleFourierGrid_includingSymmetryICOSCORRECTION(diffractionPatterns,eulerAngles,symmetryOperations,interpolationCutoffDistance)
tic%start clock
...
for diffractionPatternNumber = 1:numberOfDiffractionPatterns
    ...
    MRot2 = [ cosd(0)*cosd(2.5)*cosd(-21)-sind(0)*sind(-21) ,cosd(0)*cosd(2.5)*sind(-21)+sind(0)*cosd(-21)   ,    -cosd(0)*sind(2.5);
          -sind(0)*cosd(2.5)*cosd(-21)-cosd(0)*sind(-21), -sind(0)*cosd(2.5)*sind(-21)+cosd(0)*cosd(-21) ,   sind(0)*sind(2.5)  ;
          sind(2.5)*cosd(-21)                               , sind(2.5)*sind(-21)                                ,              cosd(2.5)];
    ...
    [ky, kx] = meshgrid(-n2:n2-1,-n2:n2-1);ky = double(ky);kx = double(kx);
        ...
        [ky, kx] = meshgrid(-n2:n2,-n2:n2);ky = double(ky);kx = double(kx);
    end
    kx = double(kx(:))';
    ...
    rotkCoords = MRot2'*R'*[kx;ky;kz];%rotate coordinates
